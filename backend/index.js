...
// Route to get the count of pending complaints
app.get("/complaints/pending/count", async (req, res) => {
  try {
    const count = await ComplaintSchema.countDocuments({ status: "Pending" });
    res.json({ count });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Failed to retrieve pending complaint count" });
  }
});

// Route to get the count of agents
app.get("/agents/count", async (req, res) => {
  try {
    const count = await UserSchema.countDocuments({ userType: "Agent" });
    res.json({ count });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Failed to retrieve agent count" });
  }
});

// Route to get the count of complaints
app.get("/complaints/count", async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).json({ error: "Failed to retrieve complaint count" });
  }
});
...
app.get("/status/:id", async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      ...
      const complaints = await ComplaintSchema.find({ userId: userId });

      // Fetch the assigned status for each complaint
      const assignedComplaints = await AssignedComplaint.find({ userId: userId });

      // Merge the assigned status into the complaints array
      const updatedComplaints = complaints.map((complaint) => {
        const assigned = assignedComplaints.find(
          (assigned) => assigned.complaintId.toString() === complaint._id.toString()
        );
        return {
          ...complaint._doc,
          assignedStatus: assigned ? assigned.status : null,
        };
      });
